module directives {
  ...
  interface IFlipswitchScope extends ng.IScope {
      on: boolean;
      off: boolean;
  }
  export function flipswitch(): ng.IDirective {
    return {
      ...
      link: ($scope: IFlipswitchScope, element: ng.IAugmentedJQuery, attributes: IFlipswitchAttributes, ngModel: ng.INgModelController) => {
        $scope.on = attributes['on'];
        $scope.off = attributes['off'];

        ngModel.$render = function() {
          $(element.children()[0]).prop('checked', ngModel.$viewValue);
        };
